...
- (NSInteger)doScanWithDeviceInfo:(NSInteger)timeout
{
    withDeviceInfo = TRUE;
...
- (NSInteger)doScanWithTimeout:(NSInteger)timeout withModelNumber:(NSString *)modelNumber andSerialNumber:(NSString *)serialNumber
{
    withDeviceInfo = TRUE;
...
- (void)connectPeripheral:(BLEOBject *)obj
{
    ...
    if (obj.connectionAttempts <= self.connectionAttempts) {
        ...
        //AW - Continue search if not looking for a particular serial/model
    ...
    if (!error) {
        ...
        for (int i = 0; i < peripheral.services.count; i++) {
            ...
            if ([service.UUID isEqual:self.iUUID]) {
                ...
                [peripheral discoverCharacteristics:@[[CBUUID UUIDWithString:@MODEL_NUM], [CBUUID UUIDWithString:@SERIAL_NUM]] forService:service];
            }
        }
    } else {
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    if (!error) {
        ...
        for (int i = 0; i < service.characteristics.count; i++) {
            ...
            if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@MODEL_NUM]]) {
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if (currentObject.modelNumber && currentObject.serialNumber) {
        ...
        if (self.serialNumber && self.modelNumber && ![currentObject.modelNumber isEqualToString:self.modelNumber] && ![currentObject.serialNumber isEqualToString:self.serialNumber]) {
            currentObject.connectionAttempts = kMaxConnectionAttempts;
        }
    }
}
